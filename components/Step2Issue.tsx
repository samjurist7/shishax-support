...
export default function Step2Issue() {
  ...
  const onDrop = useCallback(async (accepted: File[], rejected: any[]) => {
    ...
    if (files.length + accepted.length > MAX_FILES) { setFileError('Maximum 5 files. Remove one to add another.'); return }

    const previews: FP[] = accepted.map(file => ({ file, preview: file.type.startsWith('image/') ? URL.createObjectURL(file) : '', id: Math.random().toString(36).slice(2) }))
  ...
  return (
    <div className="anim-slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div>
        <label className="w-label">Description * <span style={{ color: '#555', fontWeight: 400, textTransform: 'none', letterSpacing: 0, fontSize: 12, marginLeft: 4 }}>{description.length}/1500</span></label>
        <textarea className={`w-input${issueErrors.description ? ' is-error' : ''}`} rows={5} style={{ resize: 'none', minHeight: 120 }} placeholder="Describe the issue in detail..." {...register('issue.description')} />
      ...
      <div>
        <label className="w-label">Photos / Videos * <span style={{ color: '#555', fontWeight: 400, textTransform: 'none', letterSpacing: 0, fontSize: 12, marginLeft: 4 }}>{files.length}/5</span></label>

        <div
          {...getRootProps()}
          style={{ border: `2px dashed ${isDragActive ? '#FF8000' : '#2A2A2A'}`, borderRadius: 8, padding: '24px 16px', cursor: files.length >= MAX_FILES ? 'not-allowed' : 'pointer', textAlign: 'center', transition: 'border-color 0.15s', opacity: files.length >= MAX_FILES ? 0.4 : 1, background: isDragActive ? 'rgba(255,128,0,0.05)' : 'transparent' }}
        >
          <input {...getInputProps()} />
          ...
          <p style={{ color: '#666', fontSize: 14 }}>{isDragActive ? 'Drop files here' : 'Drop photos or videos here, or click to browse. At least one is required.'}</p>
        ...
        {(fileError || issueErrors.attachments) && (
          <p className="w-error anim-fade-up" style={{ marginTop: 8 }}>
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none" style={{flexShrink:0}}><circle cx="6.5" cy="6.5" r="5.5" stroke="#F82629"/><path d="M6.5 3.5v3.5M6.5 9h.01" stroke="#F82629" strokeWidth="1.5" strokeLinecap="round"/></svg>
              ...
              <div key={fp.id} style={{ position: 'relative' }}>
                {fp.preview
                  ...
                  : <div style={{ width: 72, height: 72, borderRadius: 8, border: '1px solid #2A2A2A', background: '#1A1A1A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="24" height="24" fill="none" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z" stroke="#555" strokeWidth="1.5"/></svg>
                    </div>
                }
                <button type="button" onClick={() => removeFile(fp.id)} style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%', background: '#F82629', border: 'none', color: '#fff', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>×</button>
